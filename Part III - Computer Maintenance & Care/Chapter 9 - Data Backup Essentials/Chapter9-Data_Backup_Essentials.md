...
* Setting up Windows built-in backup (step by step)
* Cloud backup solutions that actually work
...
| :--- | :--- |
| **Hard drive failure** (40% of cases) | Cost: **$300–$3,000+** |
| **Malware/ransomware** (30% of cases) | Success rate: **30–80%** (often unsuccessful with severe ransomware) |
...
| :--- | :--- | :--- |
| **Backup** | Creates a separate copy that remains **unchanged** even if original files are deleted or modified. | You can recover a file from three months ago, or a file you accidentally deleted last week. |
| **Sync** (OneDrive, Google Drive) | Keeps files **identical** across all locations. | If you delete a file or get ransomware, the sync will **delete/encrypt your cloud copy too.** |
...
* **3** - Keep at least **3 copies** of important data.
* **2** - Store copies on **2 different types of media** (e.g., internal drive + external drive + cloud).
* **1** - Keep at least **1 copy offsite** (cloud or remote location).
...
* **Don't Need to Back Up:** Installed programs, Windows system files (can be reinstalled).
...
2.  Click **Start** and type **"File History"** (select settings).
3.  Click **"Add a drive"** and select your external drive.
4.  Click **"More options"** to configure backup frequency (Every 1-12 hours) and how long to keep backups.
5.  Click **"Back up now"** to start the initial backup.

*Pros: Free, automatic once set up, keeps multiple versions of files.*
*Cons: Local only (won't help if your house burns down), requires drive connection.*
...
| :--- | :--- | :--- |
| **Backblaze** | **Top Recommendation.** Unlimited backup for one computer ($60/year). True versioned backup. | Home users with lots of photos/videos, simple setup needed. |
| **IDrive** | Multiple computer backup under one account. Mobile device backup included. | Users with multiple devices, small home offices. |
| **CrashPlan** | Excellent server backup, legal hold features, business-grade features. | Small businesses, professional environments. |
...
* **Layer 3: Recovery (The Critical Layer):** **Offline backups** that ransomware can't reach, and **Cloud backup with versioning and retention** (not just sync).

**Safe Backup Setup:** Use an **external drive that you disconnect after backup**, and a **cloud backup service** with file versioning.
...
| User Profile | Primary Backup Plan | Secondary Backup Plan |
| :--- | :--- | :--- |
| **Basic Home User** | Windows File History to external drive (weekly connection). | OneDrive for documents (if already a subscriber). |
...
* Copy your most critical files (photos, documents) to a USB drive or manually upload them to a simple cloud storage service.
* Store the copies away from your computer.

**Phase 2: Local Backup Setup (This Week - 2 Hours)**

* Buy an external hard drive (2-4x your data size).
* Set up **Windows File History** (see detailed steps above).
* Run the initial backup.

**Phase 3: Cloud Backup (Next Week)**

* Choose and install your cloud backup service (e.g., **Backblaze**).
* Configure the service to back up your `Documents`, `Pictures`, `Videos`, and `Desktop`.
* Begin the initial upload (it will run in the background for days/weeks).

**Phase 4: Testing and Maintenance (Ongoing)**
